...
const Palgakalkulaator = () => {
    ...
    return (
        ...
        <div className="grid grid-cols-1 grid-rows-2 md:grid-cols-2 md:grid-rows-1 gap-8">
            <form className="flex flex-col justify-start gap-y-6">
                <div className="flex flex-col gap-y-2">
                    <AmountTypeSelect name="inputOption" value={state.option} onChange={handleOptionChange}/>
                    <AmountInput amount={state.amount} onChange={handleAmountChange}/>
                </div>   
                <div className="flex flex-col gap-y-2">
                    <CheckBox key={'tooandja'} check={state.isTtMEmployer} onChange={toggleTTMEmployer} text="Tööandja töötuskindlustusmakse (0.8%)" name="tooandja-kindlustusmakse"/>
                    <CheckBox key={'tootaja'} check={state.isTtMEmployee} onChange={toggleTTMEmployee} text="Töötaja (kindlustatu) töötuskindlustusmakse (1.6%)" name="tootaja-kindlustusmakse"/>
                    <PensionCheckBox value={state.isPensionCalculated} onChange={handlePensionPercentSelect} text="Kogumispension (II sammas)" name="kogumispension"/>
                </div>
            </form>
        ...
        <div className="flex justify-center items-center p-4 bg-[#FCE9E9] min-h-[100px]">
                {loading ? <div className="w-6 h-6 border-2 border-t-transparent border-[#2d0606]/60 rounded-full animate-spin"/> : <span>{aiResponse}</span>}
            </div>
        </div>
        </>
    );
